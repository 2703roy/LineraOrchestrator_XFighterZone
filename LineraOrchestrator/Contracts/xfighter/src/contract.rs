...
use xfighter::{Operation, XfighterAbi, FactoryOperation, XfighterMessage, MatchResultMessage};
...
pub struct XfighterContract {
    ...
    pending_outbound: Vec<(ChainId, XfighterMessage)>,
}
...
impl Contract for XfighterContract {
        ...
        match operation {
            ...
            Operation::Factory(factory_op) => match factory_op {
                FactoryOperation::OpenAndCreate => {
					...
					// Close chain
					if let Err(e) = self.runtime.close_chain() {
						error!("Failed to close chain: {:?}", e);
					} else {
						debug!("Chain marked closed");
					}
                }
            },
            ...
            Operation::RecordScore(input) => {
                let mt = input.match_type.clone().to_lowercase();
                if mt == "normal" {
                    info!(
                        "[XFighter] Normal match detected (match_id={}), skipping leaderboard update.",
                        input.match_id
                    );
                    // Lưu match vào state để tra cứu, nhưng KHÔNG gửi outbound
                    ...
                    let match_result_data = MatchResult {
						...
						player1_userchain: input.player1_userchain.clone(),
						player2_userchain: input.player2_userchain.clone(),
						player1_heroname: input.player1_heroname.clone(),
						player2_heroname: input.player2_heroname.clone(),
                ...
                self.state
                    .match_results
                    .insert(&chain_id_str, match_result_data.clone())
                    .expect("Failed to insert match result");
                ...
                debug!(
                    "[XFighter] Queued messages (deferred) to publisher_chain_id={:?} key={}",
                    publisher_chain_id, key
                );

                // Gửi MatchResultMessage đến publisher chain
                let match_message = MatchResultMessage {
                    chain_id: self.runtime.chain_id().to_string(),
                    match_result: match_result_data.clone(),
                };
                self.pending_outbound.push((
                    publisher_chain_id,
                    XfighterMessage::MatchResult(match_message),
                ));

                // Gửi RecordScoreMessage đến leaderboard (trên publisher chain)
                // Winner
                self.pending_outbound.push((
                    publisher_chain_id,
                    XfighterMessage::RecordScore(RecordScoreMessage {
                        user_name: input.winner_username.clone(),
                        is_winner: true,
                        match_id: match_id.clone(),
                    }),
                ));
                ...
                self.pending_outbound.push((
                    publisher_chain_id,
                    XfighterMessage::RecordScore(RecordScoreMessage {
                        user_name: loser_username.clone(),
                        is_winner: false,
                        match_id: match_id.clone(),
                    }),
                ));
                ...
                info!(
                    "[XFighter] Enqueued messages for leaderboard_app={:?}, key={}, winner={}, loser={}, match_id={}",
    ...
    /// Cross-chain Message - CHỈ MỘT METHOD NÀY
    async fn execute_message(&mut self, message: Self::Message) {
        match message {
            XfighterMessage::RecordScore(record_score_msg) => {
                ...
                info!("[XFighter] execute_message received RecordScoreMessage user={} is_winner={} match_id={}",
                    record_score_msg.user_name, record_score_msg.is_winner, record_score_msg.match_id
                );
                ...
                let op = LeaderboardOperation::RecordScore {
                    user_name: record_score_msg.user_name.clone(),
                    is_winner: record_score_msg.is_winner,
                    match_id: record_score_msg.match_id.clone(),
                };
                ...
                info!("[XFighter] Forwarded RecordScore to leaderboard app_id={:?} user={} match_id={}",
                    lb_id, record_score_msg.user_name, record_score_msg.match_id
                );
            }
            XfighterMessage::MatchResult(match_result_msg) => {
                info!("[XFighter] execute_message received MatchResultMessage from chain={}, match_id={}",
                    match_result_msg.chain_id, match_result_msg.match_result.match_id
                );

                // Lưu match result từ child chain vào state của publisher chain
                self.state
                    .match_results
                    .insert(&match_result_msg.chain_id, match_result_msg.match_result)
                    .expect("Failed to insert match result from child chain");
                    
                info!("[XFighter] Saved match result from child chain {}", match_result_msg.chain_id);
            }
        }
    }
}